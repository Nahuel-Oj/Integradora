...
{
    const [row] = await pool.query('SELECT * FROM jugadores WHERE id_jugador = ?', [id]);
    return row;
}
...
export async function getJugadoresPeso()
{
    const [row] = await pool.query ('SELECT * FROM jugadores WHERE peso BETWEEN 75 AND 80');
    return row;
}

export async function getJugadorMasAlto()
{
    const [row] = await pool.query ('SELECT * FROM jugadores ORDER BY estatura DESC LIMIT 1');
    return row;
}
...
export async function borrarJugadorById(id) 
{
    const [row] = await pool.query("DELETE FROM jugadores WHERE id_jugador = ?", [id]);
    return row;
}
